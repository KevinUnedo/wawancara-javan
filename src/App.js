...
function App() {
  ...
  const json = {
    ...
    "target_hari_ini": additionalInputsTaskHariIni.map(input => ({
      "nama_anggota": input.namaAnggota,
      "url_task": input.urlTask,
      "status_task": input.statusTask
    })),
    "task_terlambat": additionalInputsTaskTerlambat.map(input => ({
      "nama_anggota": input.namaAnggota,
      "url_task": input.urlTask,
      "alasan_terlambat": input.statusTask
    }))
  };
  ...
  return (
    <div>
      <div className="container mx-auto mt-10 px-4 pt-3 pb-4 rounded-lg border border-gray-200">
